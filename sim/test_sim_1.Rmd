...
sigma2_z <- 1 # the amount of perturbation (SD) to give to the previous time point (sigma in Sewell thesis)
for(t in 2:TT)
  for(i in 1:n)
    for(d in 1:2)
...
alpha[ , 1] <- c(5, 8, 10) # initial values (t = 1)
for(t in 2:TT) {
...
fit <- DMLPM(
  num_samples = 1e2, burn = 1e2,
  thin = 1, post_pred_thin_factor = 1e6, # large ppthin to avoid doing it, will fix later, it's too slow
  num_chains = 1,
  Y = Y,
...
chain <- 2#which.max(fit$map$i_map$log_posterior_max)
chain
...
for(k in 1:K) {
  for(t in 1:TT) {
    Y_hat <- rbind(Y_hat, # previous data frame
                   cbind(set = "sim", t, k, fit$mcmc_chains[[1]]$Y_hat[ , , t, k]), # simulated data for time t and layer k (the post. pred samples)
                   cbind(set = "obs", t, k, t(as.matrix(Y[ , , t, k] %>% upper_tri()))))
  }
}

p1 <- pivot_longer(Y_hat, cols = starts_with("V")) %>% 
  mutate(value = as.numeric(value)) %>% 
  rename(edge = name) %>% 
  group_by(set, t, k, edge) %>% 
  summarise(value = mean(value)) %>% 
  pivot_wider(names_from = set, values_from = value) %>% 
  mutate(diff = abs(obs - sim)) %>% 
  ggplot() +
  geom_abline(alpha = 0.5) +
  geom_point(aes(obs, sim), colour = "steelblue", alpha = 0.25)

p2 <- pivot_longer(Y_hat, cols = starts_with("V")) %>% 
  mutate(value = as.numeric(value)) %>% 
  rename(edge = name) %>% 
  group_by(set, t, k, edge) %>% 
  summarise(value = mean(value)) %>% 
  ...
  ggplot() +
  geom_abline(alpha = 0.5) +
  geom_point(aes(obs + 1, sim + 1), colour = "steelblue", alpha = 0.25) +
  scale_x_log10() +
  scale_y_log10()
